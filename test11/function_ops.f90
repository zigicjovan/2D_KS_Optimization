       ...
       ! Load optimal initial condition
        ...
        DO j=1,local_Ny
          DO i=1,n_nse(1)
            ...
            X = REAL(i-1,pr)*dx(1)
            Y = REAL(local_y_offset-1+j,pr)*dx(2)
            vort0(i,j) = 1.0e-14*( sin(n_nse(1)*2.0_pr*PI*((2.0_pr*PI)/Lx)*X) + sin(n_nse(2)*2.0_pr*PI*((2.0_pr*PI)/Ly)*Y) +  cos(n_nse(1)*2.0_pr*PI*((2.0_pr*PI)/Lx)*X) + cos(n_nse(2)*2.0_pr*PI*((2.0_pr*PI)/Ly)*Y) )
          END DO
        END DO
