...
!        COMPLEX(pr), DIMENSION (1:n_nse(2),1:local_Nx), INTENT(OUT) :: fu  ! Field in Fourier space
        ...
        ! Assign physical field in Fortran to a C pointer for FFT
        tmppointerR(:,:) = u(:,:)
        ! Compute forward Fourier transform
        CALL fftw_mpi_execute_dft_r2c(fwdplan, tmppointerR, tmppointerC)
        ! Assign Fourier field back to Fortran type from C pointer from FFT
        fu(:,:) = tmppointerC(:,:)
