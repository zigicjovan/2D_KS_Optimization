       ...
       ! Load optimal initial condition
        ...
        DO j=1,local_Ny
          DO i=1,n_nse(1)
            ...
            X = REAL(i-1,pr)*dx(1)
            Y = REAL(local_y_offset-1+j,pr)*dx(2)
            vort0(i,j) = ((10.0_pr)**(0))*( sin(1.0*(((2.0_pr*PI)/Lx)*X + ((2.0_pr*PI)/Ly)*Y)) + sin(5.0*((2.0_pr*PI)/Lx)*X) + sin(10.0*((2.0_pr*PI)/Ly)*Y) &
...
!      COMPLEX(pr), DIMENSION(1:n_nse(2),1:local_Nx), INTENT(OUT) :: fpsi   ! Streamfunction in Fourier space
    ...
    ! Output: fpsi - stream function in Fouier space
    !==========================================================================
    SUBROUTINE adj_streamKS(faux, gaux, fpsi)
      ! Load variables
      USE global_variables, ONLY: pr, MACH_EPSILON, n_nse, local_Nx, local_Ny, ksq
      ! Load subroutines
      USE fftwfunction, ONLY: fftfwd, fftbwd  ! FFT routines
      ! Initialize variables
!      COMPLEX(pr), DIMENSION(1:n_nse(2),1:local_Nx), INTENT(IN)  :: faux   ! Vorticity field in Fourier space
      ...
      !DO i2 = 1,local_Ny
        !DO i1 = 1,n_nse(1)
          !IF (gaux(i1, i2) > MACH_EPSILON) THEN
            !fpsi(i1,i2) = faux(i1,i2)*gaux(i1, i2)
          !fgnn(i1,i2) = -fnn(i1,i2)*gnn(i1, i2)
          !ELSE
            !fpsi(i1,i2) = 0.0_pr
          !END IF
        !END DO
